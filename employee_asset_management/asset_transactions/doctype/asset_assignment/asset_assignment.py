...
class AssetAssignment(Document):
    ...
    def validate_availability(self):
        ...
        if status != "Available":
            ...
            if current_holder:
                frappe.throw(f"This asset is already assigned to {current_holder}")
            else:
                frappe.throw(f"Asset status is '{status}'. Only 'Available' assets can be assigned.")
    ...
    def check_category_limit(self):
        ...
        if max_allowed:
            ...
            if count >= max_allowed:
                frappe.throw(f"Employee already has maximum allowed {category}. Limit: {max_allowed}")
